...
export function setupReducerBlock(action, state, actionName, stateKeys, propKey, lookUpKey) {
	...
	if (action.type === actionTypes[`${actionName}_PENDING`]) {
		states[stateKeys].pending = true;
		states[stateKeys].error = null;
		return states;
	}

	if (action.type === actionTypes[`${actionName}_SUCCESS`]) {
        ...
        states[stateKeys].data = deleted ? deleteById(array, payload.id, propKey, lookUpKey) : replacements(allArrays, payload.count, propKey, lookUpKey);
		states[stateKeys].error = null;
		return states;
	}

	if (action.type === actionTypes[`${actionName}_ERROR`]) {
		states[stateKeys].pending = null;
